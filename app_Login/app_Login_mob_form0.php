...
<HEAD>
 <TITLE><?php if ('novo' == $this->nmgp_opcao) { echo strip_tags(""); } else { echo strip_tags("Tela de Acesso"); } ?></TITLE>
...
.sc-button-image.disabled img {
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['embutida_pdf']))
 {
 ?>
...
<?php
include_once("app_Login_mob_sajax_js.php");
?>
<script type="text/javascript">
...
include_once('app_Login_mob_jquery.php');
...
<?php
$str_iframe_body = ('F' == $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['run_iframe'] || 'R' == $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['run_iframe']) ? 'margin: 2px;' : '';
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['recarga'];
}
    $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['dashboard_info']['remove_margin'] ? 'margin: 0; ' : '';
    $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['dashboard_info']['remove_border'] ? 'border-width: 0; ' : '';
    $vertical_center = 'display: flex; flex-direction: column; justify-content: center; margin: 0px; min-height: 100vh;';
?>
<body class="scFormPage" style="<?php echo $remove_margin . $str_iframe_body . $vertical_center; ?>">
<?php

if (isset($_SESSION['scriptcase']['app_Login']['error_buffer']) && '' != $_SESSION['scriptcase']['app_Login']['error_buffer'])
{
    echo $_SESSION['scriptcase']['app_Login']['error_buffer'];
}
...
<?php
 include_once("app_Login_mob_js0.php");
?>
<script type="text/javascript"> 
...
<?php
$_SESSION['scriptcase']['error_span_title']['app_Login_mob'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['app_Login_mob'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
<?php
  if (!$this->Embutida_call && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['mostra_cab']) || $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['mostra_cab'] != "N") && (!$_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['dashboard_info']['under_dashboard'] || !$_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['dashboard_info']['compact_mode'] || $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['dashboard_info']['maximized']))
  {
?>
<tr><td>
      ...
      <TABLE style="padding: 0px; border-spacing: 0px; border-width: 0px;" width="100%">
       <TR align="center" valign="middle">
        <TD align="left" rowspan="2" class="scFormHeaderFont">
          <?php if ($this->Ini->Export_img_zip) {$this->Ini->Img_export_zip[] = $this->Ini->root . $this->Ini->path_imag_cab . '/grp__NM__bg__NM__Logo_AraGestorMenor.png';echo '<IMG SRC="grp__NM__bg__NM__Logo_AraGestorMenor.png';}else{ echo '<IMG SRC="' . $this->Ini->path_imag_cab  . '/grp__NM__bg__NM__Logo_AraGestorMenor.png';}?>" BORDER="0"/>
        </TD>
...
<?php
  ...
  {
       if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['where_filter']))
       {
...
<span id="id_read_on_login" class="sc-ui-readonly-login css_login_line" style="<?php echo $sStyleReadLab_login; ?>"><?php echo $this->form_format_readonly("login", $this->form_encode_input($this->login)); ?></span><span id="id_read_off_login" class="css_read_off_login<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_login; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=44"; } ?> maxlength=255 alt="{datatype: 'text', maxLength: 255, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: '', enterTab: false, enterSubmit: true, autoTab: false, selectOnFocus: false, watermark: 'LOGIN', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
 </TD>
   <?php }?>
...
<span id="id_read_on_pswd" class="sc-ui-readonly-pswd css_pswd_line" style="<?php echo $sStyleReadLab_pswd; ?>"><?php echo $this->form_format_readonly("pswd", $this->form_encode_input($this->pswd)); ?></span><span id="id_read_off_pswd" class="css_read_off_pswd<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_pswd; ?>"><div class="scFormObjectOddPwdBox"><input class="sc-js-input scFormObjectOddPwdInput scFormObjectOddPwdText sc-ui-pwd-toggle css_pswd_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_pswd" type="password" autocomplete="off" name="pswd" value="" 
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=40"; } ?> maxlength=32 alt="{datatype: 'text', maxLength: 32, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: '', enterTab: false, enterSubmit: true, autoTab: false, selectOnFocus: false, watermark: 'SENHA', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ><span id="id_pwd_show_hide_pswd" class="sc-ui-pwd-toggle-icon"><i class="fa fa-eye sc-ui-pwd-eye" aria-hidden="true" id="id_pwd_fa_pswd"></i></span></div></span><?php } ?>
 </TD>
   <?php }?>
...
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['Lookup_remember_me'][] = '1'; ?>
...
?>
       <?php echo nmButtonOutput($this->arr_buttons, "bok", "scBtnFn_sys_format_ok()", "scBtnFn_sys_format_ok()", "sub_form_b", "", "Acessar", "" . $sCondStyle . "", "", "", "", $this->Ini->path_botoes, "", "", "sc-unique-btn-3", "", "");?>
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['masterValue']))
{
...
<script>
 var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['dashboard_info']['parent_widget']; ?>']");
 dbParentFrame[0].contentWindow.scAjaxDetailStatus("app_Login_mob");
</script>
<?php
    }
    else {
...
?>
<script>
 parent.scAjaxDetailStatus("app_Login_mob");
 parent.scAjaxDetailHeight("app_Login_mob", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
    }
}
...
?>
<script type="text/javascript">
	function scBtnFn_criar_conta() {
		if ($("#sc_criar_conta_top").length && $("#sc_criar_conta_top").is(":visible")) {
			sc_btn_criar_conta()
			 return;
		}
	}
	...
	function scBtnFn_criar_conta_empresa() {
		if ($("#sc_criar_conta_empresa_top").length && $("#sc_criar_conta_empresa_top").is(":visible")) {
			sc_btn_criar_conta_empresa()
			 return;
		}
	}
...
?>
<?php
$_SESSION['sc_session'][$this->Ini->sc_page]['app_Login_mob']['buttonStatus'] = $this->nmgp_botoes;
?>
<script type="text/javascript">
